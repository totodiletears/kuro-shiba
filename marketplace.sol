...
contract Marketplace is ERC1155Holder, ReentrancyGuard, Ownable {
    ...
    mapping(address => mapping(uint256 => Token)) public tokensForSale;
    ...
    function totalNFTContracts() public view returns (uint256) {
    ...
    function calculateFee(uint256 _price)
        public
        view
        returns (uint256 fee, uint256 toSeller)
    {
    ...
    function sellMultipleNFTs(
        ...
        for (uint256 i; i < _ids.length; i++) {
            uint256 id = _ids[i]; // gas saver
            uint256 price = _prices[i]; // gas saver
    ...
    function cancelNFTSale(address _contract, uint256 _id)
        external
        nonReentrant
    {
    ...
    function cancelMultipleNFTSales(address _contract, uint256[] calldata _ids)
        external
        nonReentrant
    {
    ...
    function purchaseNFT(
        ...
        // Data for explorer
        string memory data = _data;

        // Calculates the fee to pay to feeCollector & seller
        (uint256 fee, uint256 toSeller) = calculateFee(token.price);
